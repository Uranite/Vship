...
//dest of resize size and temp biggest of the 2 size, src of source size
void resizePlane(float* dest, float* temp, float* src, int64_t source_width, int64_t source_height, int64_t resize_width, int64_t resize_height, hipStream_t stream){
    ...
    if (source_width == resize_width && source_height == resize_height){
        GPU_CHECK(hipMemcpyDtoDAsync(dest, src, sizeof(float)*source_width*source_height, stream));
        return;
    }
